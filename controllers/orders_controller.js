...
exports.addOrder = function(req,res){
	...
	var newOrder = new Order({userid: 'Raja', items: orderItems, shipping: orderShipping, billing: orderBilling});
	newOrder.save(function(err,results){
		if(err){
			res.json(500,"Failed to save Order.");
		}else{
			Customer.update({userid: 'Raja'},{$set:{cart:[]}})
